...
export const CustomerSidebar = () => {
  ...
  const [activeFilter, setActiveFilter] = useState<'pending' | 'past' | 'cancelled'>('pending');
  const [appointments, setAppointments] = useState<Appointment[]>([]);
  const [appointmentCounts, setAppointmentCounts] = useState<AppointmentCounts>({ pending: 0, past: 0, cancelled: 0 });
  ...
  useEffect(() => {
    const fetchTranscript = async () => {
      try {
        const response = await fetch('http://localhost:3001/api/transcript/latest');
        if (response.ok) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.nudges && Array.isArray(data.nudges)) {
            ...
            setNudges((prev) => {
              const existingSids = new Set(prev.map(n => n.sid).filter(Boolean));
              ...
              if (newNudgeSids.length > 0) {
                const toAck = newNudgeSids.filter(sid => !ackedNudgeSids.current.has(sid));
                if (toAck.length > 0) {
                  // Mark as ACKed
  ...
  useEffect(() => {
    const fetchLeadScore = async () => {
      try {
        const response = await fetch('http://localhost:3001/api/lead-score/current');
        if (response.ok) {
          const data = await response.json();
          if (data.score !== null && data.score !== undefined) {
            ...
            if (customerData.phone) {
              try {
                const calcResponse = await fetch('http://localhost:3001/api/lead-score/calculate', {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      ...
      try {
        const response = await fetch(`http://localhost:3001/api/appointments?phone=${encodeURIComponent(customerData.phone)}&status=${activeFilter}`);
        if (response.ok) {
          const data = await response.json();
          setAppointments(data.appointments || []);
          setAppointmentCounts(data.counts || { pending: 0, past: 0, cancelled: 0 });
        }
      } catch (error) {
    ...
    <aside className="w-[28rem] bg-card border-l p-6 overflow-y-auto">
      <div className="space-y-6">
            ...
            <div className="space-y-2 max-h-[400px] overflow-y-auto">
              {appointments.map((appointment) => {
                ...
                return (
                  <div
                    key={appointment.id}
                    className={`border rounded-lg p-3 ${statusCardColors[appointment.status]}`}
                  >
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex-1">
                        <div className="font-semibold text-sm mb-1 text-foreground">{formattedDate}</div>
                        <div className="text-xs text-muted-foreground mb-1">{appointment.timeSlot}</div>
                        <div className="text-sm font-medium text-foreground">{appointment.service}</div>
                      </div>
                      <div className={`text-xs px-2 py-1 rounded uppercase font-medium ${statusBadgeColors[appointment.status]}`}>
                        {appointment.status}
                      </div>
                    </div>
                  </div>
                      ...
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex-1">
                          <div className="flex items-center justify-between mb-1">
                            <div className={`text-xs uppercase tracking-wide ${badgeColorClass}`}>
        ...
        <div>
          ...
          <div ref={transcriptContainerRef} className="border rounded-lg bg-muted/30 h-[400px] overflow-y-auto p-4 space-y-3">
            {transcript.length === 0 ? (
              <p className="text-sm text-muted-foreground text-center py-8">
